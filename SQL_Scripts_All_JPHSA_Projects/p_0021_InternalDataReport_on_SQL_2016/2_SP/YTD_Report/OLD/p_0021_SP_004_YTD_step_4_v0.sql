--p_0021_SP_004_YTD_step_4_v0.sql
/*
	************************************************************************************
...
SELECT @Date_End_of_YTD = '03-31-2017'
----------------------- Local Variables
DECLARE @LineNumber int
DECLARE @Source nvarchar(100)
DECLARE @Title nvarchar(255)
----------------------------------------
IF OBJECT_ID('dbo.zzz_p0021_5_IDR_part_YTD_Results') IS NOT NULL
    DROP TABLE dbo.zzz_p0021_5_IDR_part_YTD_Results


CREATE TABLE [dbo].[zzz_p0021_5_IDR_part_YTD_Results]
(
	...
	,CONSTRAINT [zzz_p0021_5_IDR_part_YTD_Results$IDR_Q_PK] PRIMARY KEY NONCLUSTERED 
		(
			[IDR_Q_PK] ASC
		)
		WITH 
...
--WHERE a.TYPE in ('PC') and a.[Check-In Day]between '20161001' and '20161031' and a.loc in ('EJ')
	...
	SELECT distinct
		  [Patient_No]
	  FROM [Success].[dbo].[zzz_p0021_6_for_Analysis_of_Part_YTD]
	  Where (
				[Type] = 'PC'
				AND
				[loc] = 'EJ'
...
INSERT INTO [Success].[dbo].[zzz_p0021_5_IDR_part_YTD_Results] 
		(
		[LineNumber],
		[Source],
		[Line_Title],
...
VALUES
		(
			...
			, ( 
					(
						SELECT TOP(1) Result 
						FROM [Success].[dbo].[zzz_p0021_5_IDR_part_YTD_Results] 
						WHERE LineNumber = 22
					)
					-
					(
...
---RESULTS TABLE
SELECT * from [Success].[dbo].[zzz_p0021_5_IDR_part_YTD_Results] 