--p_0021_SP_001_INSERT_INTO_T003_T004_and_ARCHIVEs_from_uploaded_BillableServices_v6.sql
/*
	************************************************************************************
...
--SELECT @AsOfEndOfMonth = '''05/31/2017'''			--<=== I can not see where this is being used???

DECLARE @table_name_Billable_Service_SOURCE AS sysname
SET @table_name_Billable_Service_SOURCE = N'EHSBILL_JUNE_2017'


DECLARE @table_name_NON_Billable_Service_SOURCE AS sysname
SET @table_name_NON_Billable_Service_SOURCE = N'EHSNONBILL_JUNE_2017'
...
BEGIN TRANSACTION;
		...
		SELECT 
			  [CHECKMONTH]
			  ,( SELECT RIGHT(''00'' + CAST(MONTH( [Check-In Day]) AS NVARCHAR),2)  ) as [Data_for_Month_MM]
			  ,( SELECT RIGHT(''0000'' + CAST(year( [Check-In Day]) AS NVARCHAR),4)  ) as [Data_for_Year_YYYY]
			  ,[Office Location]
			  ,[Office Name]
			  ,[Bill Provider Name]
			  ...
			  ,[CPT Description]
			  ,[Unit Qty]
			  , (0 + Convert(Char(8),Convert(datetime,[Check-In Day],101),112) - Convert(Char(8),[Patient Birth Date],112)) / 10000 as [Age_as_of_Check-In]
			  , (SELECT     ''TheCalculation'' = CASE 
													WHEN
														( ((0 + Convert(Char(8),Convert(datetime,[Check-In Day],101),112) - Convert(Char(8),[Patient Birth Date],112)) / 10000) >= 18 )
													   THEN 1 
													ELSE 0 
												END) as Adult
		...
		SELECT 
			  ( SELECT RIGHT(''00'' + CAST(MONTH([Date Encounter]) AS NVARCHAR),2)  ) as [Data_for_Month_MM]
			  ,( SELECT RIGHT(''0000'' + CAST(year([Date Encounter]) AS NVARCHAR),4)  ) as [Data_for_Year_YYYY]
			  ,[Location]
			  ,[Date Encounter]
			  ,[Patient # (Chart)]
			  ...
			  ,[Encounter Type]
			  ,[Encounter Count]
			  , (0 + Convert(Char(8),Convert(datetime,[Date Encounter],101),112) - Convert(Char(8),[Patient Birth Date (Chart)],112)) / 10000 as [Age_as_of_Date_Encounter]
			  , (SELECT     ''TheCalculation'' = CASE 
													WHEN
														( ((0 + Convert(Char(8),Convert(datetime,[Date Encounter],101),112) - Convert(Char(8),[Patient Birth Date (Chart)],112)) / 10000) >= 18 )
													   THEN 1 
													ELSE 0 
												END) as Adult
				,(SELECT     ''TheCalculation'' = 
