--p_0021_SP_001_INSERT_INTO_T003_T004_and_ARCHIVEs_from_uploaded_BillableServices_v3.sql
/*
	************************************************************************************
	...
	NOTE: This takes the data that was uploaded from the Access Database (.mdb) to the SQL Server with the table name: [Billable_Service_YYYY_MM_DD]
		  ...
		  The data in the SQL table [Billable_Service_YYYY_MM_DD] is appended to these tables in a transaction:
				...
				[T003_All_Monthly_Billable_Services_IDR]         {This table is truncated before the data is appended}
				[T003_All_Monthly_Billable_Services_IDR_ARCHIVE]

				[T004_All_Monthly_NonBillable_Services_IDR]       {This table is truncated before the data is appended}
				[T004_All_Monthly_NonBillable_Services_IDR_ARCHIVE]
...
SELECT @AsOfEndOfMonth = '''03/31/2017'''

DECLARE @table_name_Billable_Service_SOURCE AS sysname
SET @table_name_Billable_Service_SOURCE = N'zzz_9_EHSBILL_MARCH_2017_v2'


DECLARE @table_name_NON_Billable_Service_SOURCE AS sysname
SET @table_name_NON_Billable_Service_SOURCE = N'zzz_9_EHSNONBILL_MARCH_2017_v2'
...
BEGIN TRANSACTION;
		...
		TRUNCATE TABLE T003_All_Monthly_Billable_Services_IDR
				...
				IF @@ERROR <> 0
					BEGIN
						PRINT 'Truncate failed - T003_All_Monthly_Billable_Services_IDR'
					END
					ELSE
						BEGIN
							PRINT 'Truncate succeeded - T003_All_Monthly_Billable_Services_IDR'
						END						
		-- STEP 1 --------------------------END
		...
		INSERT INTO [Success].[dbo].[T003_All_Monthly_Billable_Services_IDR]
			(
			  [CHECKMONTH]
			  ,[Office_Location]
			  ,[Office_Name]
		...
		FROM [Success].[dbo].[' + @table_name_Billable_Service_SOURCE +']'
			...
			EXEC sp_executesql @stmt = @cmd_Billable_Service
					...
					IF @@ERROR <> 0
						BEGIN
							PRINT 'Insert into [T003_All_Monthly_Billable_Services_IDR] failed from: ' + @table_name_Billable_Service_SOURCE
						END
					ELSE
						BEGIN
							PRINT 'Insert into [T003_All_Monthly_Billable_Services_IDR] SUCCEEDED from: ' + @table_name_Billable_Service_SOURCE
						END
		-- STEP 2 --------------------------END
		...
		INSERT INTO [Success].[dbo].[T003_All_Monthly_Billable_Services_IDR_ARCHIVE]
			(
			  [CHECKMONTH]
			  ,[Office_Location]
			  ,[Office_Name]
		...
		FROM [Success].[dbo].[' + @table_name_Billable_Service_SOURCE +']'
			...
			EXEC sp_executesql @stmt = @cmd_ARCHIVE_Billable_Service
					...
					IF @@ERROR <> 0
						BEGIN
							PRINT 'Insert into [T003_All_Monthly_Billable_Services_IDR_ARCHIVE] failed from: ' + @table_name_Billable_Service_SOURCE
						END
					ELSE
						BEGIN
							PRINT 'Insert into [T003_All_Monthly_Billable_Services_IDR_ARCHIVE] SUCCEEDED from: ' + @table_name_Billable_Service_SOURCE
						END
		-- STEP 3 --------------------------END
		...
		TRUNCATE TABLE [T004_All_Monthly_NonBillable_Services_IDR]
				...
				IF @@ERROR <> 0
					BEGIN
						PRINT 'Truncate failed - [T004_All_Monthly_NonBillable_Services_IDR]'
					END
					ELSE
						BEGIN
							PRINT 'Truncate succeeded - [T004_All_Monthly_NonBillable_Services_IDR] '
						END						
		-- STEP 4 --------------------------END
		...
		INSERT INTO [Success].[dbo].[T004_All_Monthly_NonBillable_Services_IDR]
			(
			   [Location]
			  ,[Date_Encounter]
			  ,[Patient_No__Chart]
		...
		FROM [Success].[dbo].[' + @table_name_NON_Billable_Service_SOURCE +']'
			...
			EXEC sp_executesql @stmt = @cmd_NON_Billable_Service
					...
					IF @@ERROR <> 0
						BEGIN
							PRINT 'Insert into [T004_All_Monthly_NonBillable_Services_IDR] failed from: ' + @table_name_NON_Billable_Service_SOURCE
						END
					ELSE
						BEGIN
							PRINT 'Insert into [T004_All_Monthly_NonBillable_Services_IDR] SUCCEEDED from: ' + @table_name_NON_Billable_Service_SOURCE
						END
		-- STEP 5 --------------------------END
		...
		INSERT INTO [Success].[dbo].[T004_All_Monthly_NonBillable_Services_IDR_ARCHIVE]
			(
			   [Location]
			  ,[Date_Encounter]
			  ,[Patient_No__Chart]
		...
		FROM [Success].[dbo].[' + @table_name_NON_Billable_Service_SOURCE +']'
			...
			EXEC sp_executesql @stmt = @cmd_ARCHIVE_NON_Billable_Service
					...
					IF @@ERROR <> 0
						BEGIN
							PRINT 'Insert into [T004_All_Monthly_NonBillable_Services_IDR_ARCHIVE] failed from: ' + @table_name_NON_Billable_Service_SOURCE
						END
					ELSE
						BEGIN
							PRINT 'Insert into [T004_All_Monthly_NonBillable_Services_IDR_ARCHIVE] SUCCEEDED from: ' + @table_name_NON_Billable_Service_SOURCE
						END
		-- STEP 5 --------------------------END
...
SELECT count(*) as T003_All_Monthly_Billable_Services_IDR
  FROM [Success].[dbo].[T003_All_Monthly_Billable_Services_IDR]


SELECT count(*) AS T003_All_Monthly_Billable_Services_IDR_ARCHIVE
  FROM [Success].[dbo].[T003_All_Monthly_Billable_Services_IDR_ARCHIVE]


SELECT count(*) AS T004_All_Monthly_NonBillable_Services_IDR
  FROM [Success].[dbo].[T004_All_Monthly_NonBillable_Services_IDR]


  SELECT count(*) AS T004_All_Monthly_NonBillable_Services_IDR_ARCHIVE
