--p_0021_SP_001_B_and_C_values_Tri.sql
/*
	************************************************************************************
...
----------------------------------------
IF OBJECT_ID('dbo.zzz_InternalDataReport_part_B_and_C') IS NOT NULL
    DROP TABLE dbo.zzz_InternalDataReport_part_B_and_C


CREATE TABLE [dbo].[zzz_InternalDataReport_part_B_and_C]
(
	...
	,CONSTRAINT [zzz_InternalDataReport_part_B_and_C$IDR_a_b_PK] PRIMARY KEY NONCLUSTERED 
		(
			[IDR_a_b_PK] ASC
		)
		WITH 
...
INSERT INTO [Success].[dbo].[zzz_InternalDataReport_part_B_and_C] 
		(
		[LineNumber],
		[Source],
		[Line_Title],
...
VALUES
		(
			...
			,@Source
			,@Title 
			, (SELECT sum(Unit_Qty)FROM cte_Line_4)
		)

--SELECT * from [Success].[dbo].[zzz_InternalDataReport_part_B_and_C] 
...
VALUES
		(
			...
			, ( 
					(
						SELECT TOP(1) Result 
						FROM [Success].[dbo].[zzz_InternalDataReport_part_B_and_C] 
						WHERE LineNumber = 4
					)
					-
					(
...
VALUES
		(
			...
			,@Source
			,@Title 
			, (SELECT sum(Unit_Qty)FROM cte_Line_7)
		)
...
VALUES
		(
			...
			,@Source
			,@Title 
			, (SELECT sum(Unit_Qty)FROM cte_Line_8)
		)
...
(
	SELECT --distinct
	  ...
	  FROM [Success].[dbo].[T003_Billable_Service]
	  Where (
				Office_Name = 'JEFFCARE EAST JEFFERSON' 					--				         Office_Name = 'JEFFCARE WEST JEFFERSON'	
				AND
...
VALUES
		(
			...
			,@Source
			,@Title 
			, (SELECT sum(Unit_Qty)FROM cte_Line_35)
		)
