...
FROM         
	...
	INNER JOIN
        dbo.T002_LKNONBILL 
	ON a.[Reason for Encounter] = dbo.T002_LKNONBILL.[Encounter Reasons]
WHERE     (dbo.T002_LKNONBILL.NB = 'Y')

--===========================================================================
/*
	At this point the output from the previous SELECT is saved as a table: [EHSNONBILLA]
	The table [T001_EHSPROD] has already been dealt with. It is now named [T001_EHSPROD]
...
--4] Checking to see if the office location is present in the table [T003_Billable_Service]. You should see two records
--WB JeffCare
--EB JeffCare
...
FROM         dbo.T003_Billable_Service a INNER JOIN
                      dbo.T001_EHSPROD b ON a.[Bill Provider Name] = b.ProviderName
...
FROM         dbo.EHSNONBILLA a INNER JOIN
                      dbo.T001_EHSPROD b ON a.[Examining Clinician] = b.ProviderName
